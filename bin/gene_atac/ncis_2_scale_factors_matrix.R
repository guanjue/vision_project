...
for (i in c(1:dim(ncis_table_list)[1])){
	...
	if (t_threshold > 0){
		...
		print('scatter plot')
		png(paste(output_folder, output_file_name, '.all.log.hs.png', sep =''))
		if (log == 'T'){
			heatscatter(data_x_non0, data_y_non0, log='xy', pch = 20, ylim=c(1,10000), xlim=c(1,10000))
		} else{
		...
		print('scatter plot')
		png(paste(output_folder, output_file_name, '.all.hs.png', sep =''))
		heatscatter(data_x, data_y, pch = 20, xlim=c(20,10000))
		abline(0,1,col = 'blue')
		dev.off()
		...
		abline(h=0,col = 'red')
		###
		heatscatter(sf_info$A, sf_info$M, pch = 20, ylim=c(-10,10), main='original Signal mean', xlim=c(-1,8.5))
		abline(h=0,col = 'red')
		###
		heatscatter(sf_info_low_vs_sig$A, sf_info_low_vs_sig$M, pch = 20, ylim=c(-10,10), main='low vs sig mean', xlim=c(-1,8.5))
		abline(h=0,col = 'red')
		###
		heatscatter(sf_info_sig_vs_sig$A, sf_info_sig_vs_sig$M, pch = 20, ylim=c(-10,10), main='sig vs sig mean', xlim=c(-1,8.5))
		abline(h=0,col = 'red')
		###
		dev.off()	
