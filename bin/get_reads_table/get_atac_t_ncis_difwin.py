...
def get_ncis_t_a_b(matrix1, matrix2, output):
	...
	for w in range(1,10):
		...
		index_sample = np.random.choice(index_all, 5000000)
		for i in index_sample: ### loop matrix at x axis
			if c%100000 == 0:
				print(c)
			r1_vec = []
			r2_vec = []
			...
			if t_vec_sum in d1_tri1:
				d1_tri1[t_vec_sum].append( r1_vec_sum )
				d1_tri2[t_vec_sum].append( r2_vec_sum )
			else:
				d1_tri1[t_vec_sum] = [ r1_vec_sum ]
				d1_tri2[t_vec_sum] = [ r2_vec_sum ]
		data1.close()
		data2.close()
